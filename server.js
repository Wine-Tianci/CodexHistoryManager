...
const APP_CONFIG_PATH = path.join(__dirname, "codex-history-manager.config.json");
const PORT = process.env.PORT || 4173;
const CODEX_ROOT = process.env.CODEX_ROOT;

bootstrap().catch((error) => {
  console.error("Failed to start Codex history manager:", error.message || error);
  process.exitCode = 1;
});

async function bootstrap() {
  const appConfig = await readAppConfig({ configPath: APP_CONFIG_PATH });
  ...
  server.listen(PORT, () => {
    console.log(`Codex history manager backend listening on http://localhost:${PORT}`);
  });
}
