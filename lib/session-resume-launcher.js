...
function createSessionResumeLauncher(options = {}) {
  ...
  return async function resumeSession(sessionId) {
    ...
    const child = launchProcess(
      ...
      [
        "-NoProfile",
        "-Command",
        buildWindowsTerminalStartCommand(windowsTerminalPath, normalizedSessionId),
      ],
      {
...
function buildWindowsTerminalStartCommand(windowsTerminalPath, sessionId) {
  ...
  const argList = ["new-tab", "cmd.exe", "/k", "codex", "resume", sessionId]
    .map((value) => quotePowerShellLiteral(value))
    .join(",");
