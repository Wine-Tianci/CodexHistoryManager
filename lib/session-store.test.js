...
async function createFixture() {
  ...
  await writeJsonLines(sessionFile, [
    ...
    {
      timestamp: "2026-03-04T00:00:03Z",
      type: "event_msg",
      payload: {
        type: "token_count",
        info: {
    ...
    {
      timestamp: "2026-03-04T00:00:04Z",
      type: "event_msg",
      payload: {
        type: "token_count",
        info: {
...
test("listSessions returns summaries with fallback titles and warnings", async (t) => {
  ...
  const session = result.sessions.find((entry) => entry.sessionId === SESSION_ID);
...
test("getSessionDetail returns null usage when token_count is absent", async (t) => {
  const fixture = await createFixture();
  t.after(async () => fixture.cleanup());
  const detail = await fixture.store.getSessionDetail(ARCHIVED_ID, {
    offset: 0,
    limit: 5,
  });

  assert.strictEqual(detail.usage, null);
});
