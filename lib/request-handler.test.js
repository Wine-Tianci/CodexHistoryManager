...
test("POST /api/profiles/:id/copy creates a copied profile", async (t) => {
  const fixture = await createFixture();
  t.after(async () => fixture.cleanup());
  const created = await fixture.profileStore.createProfile({
    name: "Before",
  ...
  const response = await fetch(
    `${fixture.baseUrl}/api/profiles/${encodeURIComponent(created.id)}/copy`,
    {
      method: "POST",
    },
  );
  ...
  assert.notEqual(payload.id, created.id);
  assert.equal(payload.name, "Before copy");
  assert.equal(payload.apiKey, "sk-before");
  assert.equal(payload.baseUrl, "https://api.before.example");
  assert.equal(payload.model, "gpt-5.4");
  assert.equal(payload.modelReasoningEffort, "medium");

  const listResponse = await fetch(`${fixture.baseUrl}/api/profiles`);
  const listPayload = await listResponse.json();
  assert.equal(listPayload.profiles.length, 2);
});
...
test("POST /api/claude/profiles/:id/copy creates a copied Claude profile", async (t) => {
  const fixture = await createFixture();
  t.after(async () => fixture.cleanup());
  const created = await fixture.claudeProfileStore.createProfile({
    name: "Claude Bedrock",
    baseUrl: "https://example.test/bedrock",
    apiKey: "sk-claude",
    defaultModel: "claude-haiku-4-5-20251001",
  ...
  const response = await fetch(
    `${fixture.baseUrl}/api/claude/profiles/${encodeURIComponent(created.id)}/copy`,
    {
      method: "POST",
    },
  );
  ...
  assert.equal(response.status, 201);
  assert.notEqual(payload.id, created.id);
  assert.equal(payload.name, "Claude Bedrock copy");
  assert.equal(payload.baseUrl, "https://example.test/bedrock");
  assert.equal(payload.apiKey, "sk-claude");
  assert.equal(payload.defaultModel, "claude-haiku-4-5-20251001");

  const listResponse = await fetch(`${fixture.baseUrl}/api/claude/profiles`);
  const listPayload = await listResponse.json();
  assert.equal(listPayload.profiles.length, 2);
});
