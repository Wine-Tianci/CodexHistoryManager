...
test("copyProfile creates an identical saved profile with a copy suffix", async (t) => {
  const fixture = await createFixture();
  t.after(async () => fixture.cleanup());
  ...
  const copied = await fixture.store.copyProfile(created.id);
  ...
  assert.deepEqual(
    result.profiles.map((profile) => profile.name).sort(),
    ["Work", "Work copy"],
  );
});
...
test("listProfiles does not rewrite saved profiles when the live Codex model drifts", async (t) => {
  const fixture = await createFixture();
  t.after(async () => fixture.cleanup());
  ...
  await fs.writeFile(
    path.join(fixture.base, "config.toml"),
    [
      'model_provider = "ProxyManaged"',
      'model = "gpt-5.6"',
      "",
      "[model_providers.ProxyManaged]",
      'name = "ProxyManaged"',
  ...
  await fs.writeFile(
    path.join(fixture.base, "auth.json"),
    JSON.stringify(
      { auth_mode: "apikey", OPENAI_API_KEY: "PROXY_MANAGED" },
      null,
      2,
    ),
    "utf8",
  ...
  const refreshed = await fixture.store.listProfiles();
  const persisted = JSON.parse(
    await fs.readFile(path.join(fixture.base, "ai-agent-deck.profiles.json"), "utf8"),
  );

  assert.equal(refreshed.activeProfileId, created.id);
  assert.equal(refreshed.profiles.find((profile) => profile.id === created.id)?.isActive, true);
  assert.equal(refreshed.currentConfig.model, "gpt-5.6");
  assert.equal(refreshed.profiles[0].provider, "Custom");
  assert.equal(refreshed.profiles[0].apiKey, "sk-before");
  assert.equal(refreshed.profiles[0].baseUrl, "https://api.before.example");
  assert.equal(persisted.profiles[0].model, "gpt-5.4");
  assert.equal(persisted.profiles[0].provider, "Custom");
  assert.equal(persisted.profiles[0].apiKey, "sk-before");
  assert.equal(persisted.profiles[0].baseUrl, "https://api.before.example");
});

test("updateProfile syncs the live model into the saved active profile", async (t) => {
  const fixture = await createFixture();
  t.after(async () => fixture.cleanup());
  ...
  await fs.writeFile(
    ...
    [
      ...
      "[model_providers.Custom]",
      'name = "Custom"',
      'base_url = "https://api.credentials.example"',
      'wire_api = "responses"',
      "",
    ].join("\n"),
    "utf8",
  ...
  assert.equal(refreshed.activeProfileId, created.id);
  assert.equal(updated.model, "gpt-5.6");
  assert.equal(refreshed.profiles.find((profile) => profile.id === created.id)?.model, "gpt-5.6");
  assert.equal(persisted.profiles[0].model, "gpt-5.6");
});
