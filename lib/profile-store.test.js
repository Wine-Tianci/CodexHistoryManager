...
test("createProfile and updateProfile persist saved profiles", async (t) => {
  ...
  await assert.doesNotReject(
    fs.access(path.join(fixture.base, "codex-manager.profiles.json")),
  );
});

test("default profile store reads legacy file and writes renamed file", async (t) => {
  const fixture = await createFixture();
  t.after(async () => fixture.cleanup());

  await fs.writeFile(
    path.join(fixture.base, "codex-history-manager.profiles.json"),
    JSON.stringify(
      {
        version: 1,
  ...
  assert.equal(listed.profiles[0].name, "Legacy");
  ...
  const renamedStore = JSON.parse(
    await fs.readFile(path.join(fixture.base, "codex-manager.profiles.json"), "utf8"),
  );
  assert.deepEqual(
    renamedStore.profiles.map((profile) => profile.name),
    ["Legacy", "New"],
  );
});
