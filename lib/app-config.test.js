...
test("readAppConfig returns terminalPath from a valid config file", async (t) => {
  ...
  const configPath = path.join(tempDir, "codex-history-manager.config.json");
  await fs.writeFile(
    configPath,
    JSON.stringify({
...
test("readAppConfig returns defaults when the config file is missing", async () => {
  const config = await readAppConfig({
    configPath: path.join(os.tmpdir(), "definitely-missing-codex-history-manager.config.json"),
  });
