...
async function createTempDir() {
  return fs.mkdtemp(path.join(os.tmpdir(), "ai-agent-deck-app-config-"));
}
...
test("readAppConfig returns terminalPath from a valid config file", async (t) => {
  ...
  const configPath = path.join(tempDir, "ai-agent-deck.config.json");
  await fs.writeFile(
    configPath,
    JSON.stringify({
...
test("readAppConfig returns defaults when the config file is missing", async () => {
  const config = await readAppConfig({
    configPath: path.join(os.tmpdir(), "definitely-missing-ai-agent-deck.config.json"),
  });
...
test("readAppConfig returns defaults instead of reading old project config names", async (t) => {
  const tempDir = await createTempDir();
  t.after(() => fs.rm(tempDir, { recursive: true, force: true }));

  const configPath = path.join(tempDir, "ai-agent-deck.config.json");
  await fs.writeFile(
    path.join(tempDir, "codex-manager.config.json"),
    JSON.stringify({
