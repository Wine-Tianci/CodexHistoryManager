...
class ProfileStore {
  ...
  async updateProfile(profileId, patch) {
    const data = await this.#readProfileStore();
    const existingProfile = data.profiles.find((profile) => profile.id === profileId);
    if (!existingProfile) {
      throw new Error(`profile ${profileId} not found`);
    }
    ...
    try {
      currentConfig = await this.readCurrentConfig();
    } catch {
      currentConfig = null;
    }

    const activeProfileId = currentConfig
      ...
      : null;
    const submittedModel = normalizeOptionalString(
      patch?.model ?? existingProfile.model,
    );
    const shouldSyncCurrentModel =
      activeProfileId === profileId &&
      submittedModel === existingProfile.model;

    const updatedProfile = {
      id: existingProfile.id,
      name: normalizeRequiredString(patch?.name ?? existingProfile.name, "name"),
      provider: normalizeProfileProvider(patch?.provider ?? existingProfile.provider),
      apiKey: normalizeRequiredString(patch?.apiKey ?? existingProfile.apiKey, "apiKey"),
      baseUrl: normalizeRequiredString(patch?.baseUrl ?? existingProfile.baseUrl, "baseUrl"),
      model: shouldSyncCurrentModel
        ? currentConfig.model
        : normalizeOptionalString(patch?.model ?? existingProfile.model),
      modelReasoningEffort: normalizeOptionalString(
        patch?.modelReasoningEffort ?? existingProfile.modelReasoningEffort,
      ),
    };
...
function resolveActiveProfileId(profiles, lastActivatedProfileId, currentConfig) {
  const exactActiveProfileId = findActiveProfileId(profiles, currentConfig);
  if (exactActiveProfileId) {
    return exactActiveProfileId;
  }
  if (!lastActivatedProfileId) {
    return null;
  }
  const fallbackProfile = profiles.find(
    (item) => item.id === lastActivatedProfileId,
  );
  if (!fallbackProfile) {
    return null;
  }
  return fallbackProfile.model !== currentConfig.model ? fallbackProfile.id : null;
}
